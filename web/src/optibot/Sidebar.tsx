...
export function Sidebar({ tab, onTabChange }: SidebarProps) {
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 8px 28px 8px' }}>
        <div
          style={{
            width: 34,
            height: 34,
        ...
        />
        <div style={{ fontSize: 18, fontWeight: 700, letterSpacing: '-0.02em', color: '#0f2e29' }}>OptiBot</div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button key={item.tab} type="button" onClick={() => onTabChange(item.tab)} style={navStyle(active)}>
      ...
      <div style={{ marginTop: 'auto', padding: '12px 12px', borderRadius: 12, background: '#f4fbf9' }}>
        <div style={{ fontSize: 12, color: '#5c8a80', lineHeight: 1.5 }}>Signed in as</div>
        <div style={{ fontSize: 13, fontWeight: 600, color: '#0f2e29' }}>ops-team@company</div>
      </div>
    </div>
  )
}
